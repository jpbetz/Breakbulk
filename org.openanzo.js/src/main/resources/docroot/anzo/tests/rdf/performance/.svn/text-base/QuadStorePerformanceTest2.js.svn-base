...
dojo.provide("anzo.tests.rdf.performance.QuadStorePerformanceTest2");
...
tests.register("anzo.tests.rdf.performance.QuadStorePerformanceTest2", 
	[
		...
		function test_Performance1() {
		    ...
		    console.debug("   This test adds statements that have the same subjects, but don't share any of the predicates, objects or named graphs.");
		    
		    var store = new anzo.rdf.QuadStore();
		     
		    var patterns = anzo.tests.utilities.addStmts(store, NUM_STMTS, 1, NUM_STMTS, NUM_STMTS, NUM_STMTS);
            console.debug("----------------------------------------------");
