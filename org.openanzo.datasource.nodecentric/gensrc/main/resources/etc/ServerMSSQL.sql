...
#
#
initDBtables=CREATE TABLE GLITTERUNIT ( FAKE INT NOT NULL ) {2}  ;; CREATE INDEX GLIT_INDX ON GLITTERUNIT(FAKE) {1};; INSERT INTO GLITTERUNIT (FAKE) VALUES (0);; CREATE TABLE {0}_U ( ID   BIGINT IDENTITY (1152921504606846977,1), VALUE  {7}({4}) {3}, REF   {6} NOT NULL, PRIMARY KEY(ID) )  {2};; CREATE TABLE {0}_B ( ID   BIGINT IDENTITY (1,1), VALUE  {7}({4}) {3}, REF   {6} NOT NULL, PRIMARY KEY(ID) )  {2};; CREATE TABLE {0}_L ( ID    BIGINT IDENTITY (2305843009213693953,1), VALUE  {7}({4}) {3}, MODIFIER_ID {6}  NOT NULL, REF   {6} NOT NULL, PRIMARY KEY(ID) )  {2};; CREATE TABLE {0}_TL ( ID    BIGINT IDENTITY (4611686018427387905,1), VALUE  {7}({4}) {3}, MODIFIER_ID {6}  NOT NULL, REF   {6} NOT NULL, PRIMARY KEY(ID) )  {2};; CREATE TABLE {0}_LL ( ID   BIGINT IDENTITY (3458764513820540929,1), HASH  {6} NOT NULL, VALUE  {8}, MODIFIER_ID {6}, REF   {6} NOT NULL, PRIMARY KEY (ID) ) {2};; CREATE TABLE {0}_LTL ( ID   BIGINT IDENTITY (5764607523034234881,1), HASH  {6} NOT NULL, VALUE  {8}, MODIFIER_ID {6}, REF   {6} NOT NULL, PRIMARY KEY (ID) ) {2};; CREATE TABLE {0}_LU ( ID   BIGINT IDENTITY (6917529027641081857,1), HASH  {6} NOT NULL, VALUE  {8}, REF   {6} NOT NULL, PRIMARY KEY (ID) ) {2};; CREATE TABLE {0}_USED_IDS ( ID            {6} NOT NULL, TRANSACTIONID {6} NOT NULL )  {2};; CREATE TABLE {0}_DATATYPE ( ID   {9} NOT NULL, VALUE  {7}({4}) {3}, PRIMARY KEY(ID) )  {2} ;; CREATE TABLE {0}_LANGUAGE ( ID   {9} NOT NULL, VALUE  {7}({4}) {3}, PRIMARY KEY(ID) )  {2};; CREATE TABLE STATEMENTS ( ID                  {7}({4}) {3} NOT NULL , METADATA    {5} NOT NULL, UUID     {6} NOT NULL, NAMEDGRAPHID   {6} NOT NULL, SUBJECT        {6} NOT NULL, PREDICATE      {6} NOT NULL, OBJECT         {6} NOT NULL, RSTART                 {6} NOT NULL, REND     {6} , COMMITTED               {6} NOT NULL );; CREATE TABLE STATEMENTS_NR ( ID                  {7}({4}) {3} NOT NULL , METADATA    {5} NOT NULL, NAMEDGRAPHID   {6} NOT NULL, SUBJECT        {6} NOT NULL, PREDICATE      {6} NOT NULL, OBJECT         {6} NOT NULL, COMMITTED               {6} NOT NULL, PRIMARY KEY    (ID) );; CREATE TABLE NAMEDGRAPHS ( ID              {6} NOT NULL, METAID   {6} NOT NULL, UUID   {6} NOT NULL, REVISION  {6} NOT NULL, HSTART         {6} NOT NULL, HEND         {6}, LASTMODIFIEDBY {6} NOT NULL, COMMITTED       {6} NOT NULL, PRIMARY KEY  (UUID,REVISION) );; CREATE TABLE NAMEDGRAPHS_NR ( ID              {6} NOT NULL, METAID   {6} NOT NULL, UUID   {6} NOT NULL, REVISION  {6} NOT NULL, HSTART         {6} NOT NULL, LASTMODIFIEDBY {6} NOT NULL, COMMITTED       {6} NOT NULL, PRIMARY KEY  (UUID,REVISION) );; CREATE TABLE LASTTRANSACTIONTIME ( ID    {6} NOT NULL, COMMITED  {6} NOT NULL, PRIMARY KEY (ID) );; CREATE TABLE TRANSACTIONTIME ( ID    {6} NOT NULL, SERVERID  {7}({4}) {3} NOT NULL, COMMITED  {6}, URI    {6}, CONTEXT   {8}, PRIMARY KEY (ID) );; CREATE TABLE LOCKED_GRAPHS ( ID              {6} NOT NULL, TRANSACTIONID {6} NOT NULL, PRIMARY KEY  (ID) );; CREATE TABLE QUERY_GRAPHS( ID   {6} NOT NULL, DSID  {6} NOT NULL, PRIMARY KEY (ID,DSID) );; CREATE VIEW ALL_STMTS_VIEW (METADATA,NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT,COMMITTED) AS SELECT ST.METADATA,ST.NAMEDGRAPHID,ST.SUBJECT,ST.PREDICATE,ST.OBJECT,COMMITTED FROM STATEMENTS ST WHERE COMMITTED<=0 AND ST.REND IS NULL UNION SELECT ST.METADATA,ST.NAMEDGRAPHID,ST.SUBJECT,ST.PREDICATE,ST.OBJECT,COMMITTED FROM STATEMENTS_NR ST WHERE COMMITTED<=0;; 
initIndexes=CREATE INDEX SRUUID ON STATEMENTS (UUID,COMMITTED,RSTART,REND) INCLUDE (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT);; CREATE INDEX SRHS ON STATEMENTS (SUBJECT,PREDICATE) WHERE  COMMITTED=0 AND REND IS NULL ;; CREATE INDEX SRHP ON STATEMENTS (PREDICATE,OBJECT) WHERE  COMMITTED=0 AND REND IS NULL;; CREATE INDEX SRHO ON STATEMENTS (OBJECT)  WHERE  COMMITTED=0 AND REND IS NULL;; CREATE INDEX SRHC ON STATEMENTS (COMMITTED DESC,ID) ;; CREATE INDEX SRIE ON STATEMENTS (REND,ID);; CREATE INDEX SIRC ON STATEMENTS (ID) WHERE COMMITTED=0 AND REND IS NULL;; CREATE INDEX SRHSSPOIO ON STATEMENTS (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT,METADATA,REND,RSTART,ID);; CREATE INDEX SRHSSPOIO_NR ON STATEMENTS_NR (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT,METADATA,ID);; CREATE INDEX SRHS_NR ON STATEMENTS_NR (SUBJECT);; CREATE INDEX SRHP_NR ON STATEMENTS_NR (PREDICATE);; CREATE INDEX SRHO_NR ON STATEMENTS_NR (OBJECT);; CREATE INDEX SRHC_NR ON STATEMENTS_NR (COMMITTED);; CREATE INDEX NG_ITRM ON NAMEDGRAPHS (ID,HEND,COMMITTED,HSTART,REVISION,METAID);; CREATE INDEX NG_MH ON NAMEDGRAPHS (METAID,HEND,HSTART, ID);; CREATE INDEX NG_C ON NAMEDGRAPHS (COMMITTED);; CREATE INDEX NG_RHI ON NAMEDGRAPHS(REVISION,HSTART,ID);; CREATE INDEX NG_UUID ON NAMEDGRAPHS(UUID,HEND,ID);; CREATE INDEX NG_LM ON NAMEDGRAPHS(LASTMODIFIEDBY);; CREATE INDEX NG_IHR_NR ON NAMEDGRAPHS_NR (ID,COMMITTED,HSTART,REVISION);; CREATE INDEX NG_M_NR ON NAMEDGRAPHS_NR (METAID);; CREATE INDEX NG_RH_NR ON NAMEDGRAPHS_NR(REVISION,HSTART);; CREATE INDEX NG_UUIDR ON NAMEDGRAPHS_NR(UUID,ID);; CREATE INDEX NG_LMR ON NAMEDGRAPHS_NR(LASTMODIFIEDBY);; CREATE INDEX NG_C_NR ON NAMEDGRAPHS_NR (COMMITTED);; CREATE INDEX LTT ON LASTTRANSACTIONTIME (ID,COMMITED);; CREATE INDEX LTT4 ON LASTTRANSACTIONTIME (COMMITED,ID);; CREATE INDEX TID ON TRANSACTIONTIME (ID,COMMITED);; CREATE INDEX TCIU ON TRANSACTIONTIME (COMMITED,ID,URI);; CREATE INDEX TCSID ON TRANSACTIONTIME(SERVERID,COMMITED);; CREATE UNIQUE INDEX {0}_U_V ON {0}_U(VALUE);; CREATE INDEX {0}_U_RI ON {0}_U(REF DESC,ID);; CREATE INDEX {0}_U_RV ON {0}_U(REF ,VALUE,ID);; CREATE INDEX {0}_LU_V ON {0}_LU(HASH,ID);; CREATE INDEX {0}_LU_RI ON {0}_LU(REF DESC,ID);; CREATE UNIQUE INDEX {0}_L_V ON {0}_L(VALUE,MODIFIER_ID);; CREATE INDEX {0}_L_RI ON {0}_L(REF DESC,ID);; CREATE INDEX {0}_L_RV ON {0}_L(REF,MODIFIER_ID ,VALUE,ID);; CREATE UNIQUE INDEX {0}_TL_V ON {0}_TL(VALUE,MODIFIER_ID);; CREATE INDEX {0}_TL_RI ON {0}_TL(REF DESC,ID);; CREATE INDEX {0}_TL_RV ON {0}_TL(REF,MODIFIER_ID ,VALUE,ID);; CREATE INDEX {0}_LL_V ON {0}_LL(HASH,MODIFIER_ID);; CREATE INDEX {0}_LL_RI ON {0}_LL(REF DESC,ID);; CREATE INDEX {0}_LTL_V ON {0}_LTL(HASH,MODIFIER_ID);; CREATE INDEX {0}_LTL_RI ON {0}_LTL(REF DESC,ID);; CREATE UNIQUE INDEX {0}_B_V ON {0}_B(VALUE);; CREATE INDEX {0}_B_RI ON {0}_B(REF DESC,ID);; CREATE INDEX {0}_B_RV ON {0}_B(REF ,VALUE,ID);; CREATE INDEX {0}_UIDS_TI ON  {0}_USED_IDS(TRANSACTIONID,ID)   {1};; CREATE INDEX {0}_UIDS_IT ON  {0}_USED_IDS(ID,TRANSACTIONID)   {1};; CREATE UNIQUE INDEX LOCKEDS ON LOCKED_GRAPHS(TRANSACTIONID,ID);; CREATE UNIQUE INDEX LOCKEDID ON LOCKED_GRAPHS(ID);; CREATE UNIQUE INDEX QG_DSID_ID ON QUERY_GRAPHS(DSID,ID);; CREATE UNIQUE INDEX {0}_DATATYPE_VI ON {0}_DATATYPE(VALUE,ID)  {1};; CREATE UNIQUE INDEX {0}_DATATYPE_IV ON {0}_DATATYPE(ID,VALUE)  {1};; CREATE UNIQUE INDEX {0}_LANGUAGE_VI ON {0}_LANGUAGE(VALUE,ID)  {1};; CREATE UNIQUE INDEX {0}_LANGUAGE_IV ON {0}_LANGUAGE(ID,VALUE)  {1} ;; 
initSequences=
...
initTablespace=
createTemporaryTables=CREATE TABLE #DEFAULTGRAPHS_TMP ( ID {6} NOT NULL );; CREATE INDEX DEFAULTGRAPHS_IDX ON #DEFAULTGRAPHS_TMP (ID);; CREATE TABLE #NAMEDGRAPHS_TMP ( ID {6} NOT NULL );; CREATE INDEX  NAMEDGRAPHS_IDX ON #NAMEDGRAPHS_TMP (ID);; CREATE TABLE #TEMPGRAPHS ( ID {6} NOT NULL );; CREATE INDEX  TEMPGRAPHS_IDX ON #TEMPGRAPHS (ID);; CREATE TABLE #TEMP_COLUMNS( C0    {6}, C1    {6}, C2    {6}, C3    {6}, C4    {6}, C5    {6}, C6    {6}, C7    {6}, C8    {6}, C9    {6}, C10    {6}, C11    {6}, C12    {6}, C13    {6}, C14    {6}, C15    {6} );; CREATE TABLE #TEMP_CONSTRAINT0( ID    {6} );; CREATE INDEX #CONSTRAINT0_IDX ON #TEMP_CONSTRAINT0 (ID);; CREATE TABLE #TEMP_CONSTRAINT1( ID    {6} );; CREATE INDEX #CONSTRAINT1_IDX ON #TEMP_CONSTRAINT1 (ID);; CREATE TABLE #TEMP_CONSTRAINT2( ID    {6} );; CREATE INDEX #CONSTRAINT2_IDX ON #TEMP_CONSTRAINT2 (ID);; CREATE TABLE #TEMP_CONSTRAINT3( ID    {6} );; CREATE INDEX #CONSTRAINT3_IDX ON #TEMP_CONSTRAINT3 (ID);; CREATE TABLE #NGR_TMP( ID     {6}, METAID   {6}, REVISION   {6}, NEW_REVISION {6}, UUID   {6} ) ;; CREATE TABLE #STMTS_REP_TMP ( ID     {7}({4}) {3} NOT NULL, NAMEDGRAPHID  {6}, SUBJECT       {6}, PREDICATE     {6}, OBJECT        {6}, RSTART    {6}, REND    {6} )  ;; CREATE INDEX STMTS_REP_INDEX1 ON #STMTS_REP_TMP(NAMEDGRAPHID,REND);; CREATE TABLE #SUBJECT_IDS_TEMP ( ID  {6} );; CREATE TABLE #PREDICATE_IDS_TEMP ( ID  {6} );; CREATE TABLE #OBJECT_IDS_TEMP ( ID  {6} );; CREATE TABLE #NAMEDGRAPH_IDS_TEMP ( ID  {6} );; CREATE TABLE #ID_TMP ( ID  {6}, ENTRYID {6}, TYPE  {5} ) ;; CREATE TABLE #RES_TMP ( ENTRYID {6}, VALUE {7}({4}) {3} ) ;; CREATE INDEX  RES_TMP1 ON #RES_TMP(VALUE,ENTRYID);; CREATE TABLE #LIT_TMP ( ENTRYID {6}, VALUE  {7}({4}) {3}, MODIFIER_ID {6} ) ;; CREATE INDEX LIT_TMP1 ON #LIT_TMP(VALUE,MODIFIER_ID,ENTRYID);; CREATE TABLE #STMTS_TMP ( OPERATION   {5} NOT NULL, ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL, METADATA   {5} NOT NULL, UUID    {6} NOT NULL, NAMEDGRAPHID  {6} NOT NULL, SUBJECT         {6} NOT NULL, PREDICATE        {6} NOT NULL, OBJECT          {6} NOT NULL, REVISION   {6} , COMMITTED   {6} NOT NULL ) ;; CREATE INDEX STMTS1 ON #STMTS_TMP(OPERATION,ID);; CREATE INDEX STMTS2 ON #STMTS_TMP(ID,OPERATION);; CREATE INDEX STMTS3 ON #STMTS_TMP(STMTID,ID);; CREATE TABLE #STMT_ID_TMP ( ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL ) ;; CREATE INDEX STMT_ID_1 ON #STMT_ID_TMP(STMTID,ID);; CREATE TABLE #REMOVE_GRAPHS_TMP ( ID     {6} NOT NULL, TYPE    {5} NOT NULL, REND    {6} NOT NULL ) ;; CREATE INDEX RG_ID ON #REMOVE_GRAPHS_TMP(TYPE,ID,REND);; 
...
8To9more=
10To11=
10To11more=DROP INDEX STATEMENTS.SRUUID;; DROP INDEX STATEMENTS.SRHS;; DROP INDEX STATEMENTS.SRHP;; DROP INDEX STATEMENTS.SRHO;; DROP INDEX STATEMENTS.SRHC;; DROP INDEX STATEMENTS.SRI;; CREATE INDEX SRUUID ON STATEMENTS (UUID,COMMITTED,RSTART,REND) INCLUDE (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT);; CREATE INDEX SRHS ON STATEMENTS (SUBJECT,PREDICATE) WHERE  COMMITTED=0 AND REND IS NULL ;; CREATE INDEX SRHP ON STATEMENTS (PREDICATE,OBJECT) WHERE  COMMITTED=0 AND REND IS NULL;; CREATE INDEX SRHO ON STATEMENTS (OBJECT)  WHERE  COMMITTED=0 AND REND IS NULL;; CREATE INDEX SRHC ON STATEMENTS (COMMITTED DESC,ID) ;; CREATE INDEX SIRC ON STATEMENTS (ID) WHERE COMMITTED=0 AND REND IS NULL;; DROP INDEX STATEMENTS.SMETA_NR;; DROP INDEX ANZO_U.ANZO_U_RI;; DROP INDEX ANZO_LU.ANZO_LU_RI;; DROP INDEX ANZO_L.ANZO_L_RI;; DROP INDEX ANZO_TL.ANZO_TL_RI;; DROP INDEX ANZO_LL.ANZO_LL_RI;; DROP INDEX ANZO_LTL.ANZO_LTL_RI;; DROP INDEX ANZO_B.ANZO_B_RI;; CREATE INDEX ANZO_U_RI ON ANZO_U(REF DESC,ID);; CREATE INDEX ANZO_U_RV ON ANZO_U(REF ,VALUE);; CREATE INDEX ANZO_LU_RI ON ANZO_LU(REF DESC,ID);; CREATE INDEX ANZO_L_RI ON ANZO_L(REF DESC,ID);; CREATE INDEX ANZO_L_RV ON ANZO_L(REF ,VALUE,MODIFIER_ID);; CREATE INDEX ANZO_TL_RI ON ANZO_TL(REF DESC,ID);; CREATE INDEX ANZO_TL_RV ON ANZO_TL(REF ,VALUE,MODIFIER_ID);; CREATE INDEX ANZO_LL_RI ON ANZO_LL(REF DESC,ID);; CREATE INDEX ANZO_LTL_RI ON ANZO_LTL(REF DESC,ID);; CREATE INDEX ANZO_B_RI ON ANZO_B(REF DESC,ID);; CREATE INDEX ANZO_B_RV ON ANZO_B(REF ,VALUE);; 
...
InsertStatements.commitRemoveStatements=UPDATE STATEMENTS SET COMMITTED=?, REND=ST.REVISION FROM {0}STMTS_TMP ST WHERE STATEMENTS.COMMITTED=0 AND STATEMENTS.REND IS NULL AND ST.OPERATION=0 AND ST.ID=STATEMENTS.ID
...
InsertStatements.commitRemoveStatementsRange=UPDATE STATEMENTS SET COMMITTED=?, REND=(SELECT ST.REVISION FROM {0}STMTS_TMP ST WHERE ST.OPERATION=0 AND ST.ID=STATEMENTS.ID) FROM {0}STMTS_TMP ST WHERE STATEMENTS.COMMITTED=0 AND STATEMENTS.REND IS NULL AND ST.STMTID >= ? AND ST.STMTID < ? AND ST.OPERATION=0 AND ST.ID=STATEMENTS.ID
...
InsertStatements.commitRemoveStatementsNR=UPDATE STATEMENTS_NR SET COMMITTED=? FROM {0}STMTS_TMP ST WHERE STATEMENTS_NR.COMMITTED=0 AND ST.ID=STATEMENTS_NR.ID AND ST.OPERATION=0
...
InsertStatements.commitRemoveStatementsNRRange=UPDATE STATEMENTS_NR SET COMMITTED=? FROM FROM {0}STMTS_TMP ST WHERE STATEMENTS_NR.COMMITTED=0 AND ST.STMTID >= ? AND ST.STMTID < ? AND ST.OPERATION=0 AND ST.ID=STATEMENTS_NR.ID
...
NamedGraph.deleteStatementsForNamedGraphBatch=UPDATE STATEMENTS SET REND={0}{1}.REND FROM {0}{1} WHERE STATEMENTS.REND IS NULL AND {0}{1}.TYPE=0 AND {0}{1}.ID=STATEMENTS.NAMEDGRAPHID
...
LastTransaction.selectCurrentTimestamp=NULL
...
Server.lockTable=
Server.stats=UPDATE STATISTICS {0}
