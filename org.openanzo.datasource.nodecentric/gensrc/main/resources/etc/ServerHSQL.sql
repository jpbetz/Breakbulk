...
initTablespace=
createTemporaryTables=CREATE TEMPORARY TABLE DEFAULTGRAPHS_TMP ( ID {6} NOT NULL ) ;; CREATE INDEX DEFAULTGRAPHS_IDX ON DEFAULTGRAPHS_TMP (ID);; CREATE TEMPORARY TABLE NAMEDGRAPHS_TMP ( ID {6} NOT NULL ) ;; CREATE INDEX NAMEDGRAPHS_IDX ON NAMEDGRAPHS_TMP (ID);; CREATE TEMPORARY TABLE TEMPGRAPHS ( ID {6} NOT NULL ) ;; CREATE INDEX TEMPGRAPHS_IDX ON TEMPGRAPHS (ID);; CREATE TEMPORARY TABLE TEMP_COLUMNS( C0    {6}, C1    {6}, C2    {6}, C3    {6}, C4    {6}, C5    {6}, C6    {6}, C7    {6}, C8    {6}, C9    {6}, C10    {6}, C11    {6}, C12    {6}, C13    {6}, C14    {6}, C15    {6} );; CREATE TEMPORARY TABLE TEMP_CONSTRAINT0( ID    {6} );; CREATE INDEX CONSTRAINT0_IDX ON TEMP_CONSTRAINT0 (ID);; CREATE TEMPORARY TABLE TEMP_CONSTRAINT1( ID    {6} );; CREATE INDEX CONSTRAINT1_IDX ON TEMP_CONSTRAINT1 (ID);; CREATE TEMPORARY TABLE TEMP_CONSTRAINT2( ID    {6} );; CREATE INDEX CONSTRAINT2_IDX ON TEMP_CONSTRAINT2 (ID);; CREATE TEMPORARY TABLE TEMP_CONSTRAINT3( ID    {6} );; CREATE INDEX CONSTRAINT3_IDX ON TEMP_CONSTRAINT3 (ID);; CREATE TEMPORARY TABLE NGR_TMP( ID     {6}, METAID   {6}, REVISION   {6}, NEW_REVISION {6}, UUID   {6} )  ;; CREATE TEMPORARY TABLE STMTS_REP_TMP ( ID     {7}({4}) {3} NOT NULL, NAMEDGRAPHID  {6}, SUBJECT       {6}, PREDICATE     {6}, OBJECT        {6}, RSTART    {6}, REND    {6} )  ;; CREATE TEMPORARY TABLE SUBJECT_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE TEMPORARY TABLE PREDICATE_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE TEMPORARY TABLE OBJECT_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE TEMPORARY TABLE NAMEDGRAPH_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE TEMPORARY TABLE ID_TMP ( ID  {6}, ENTRYID {6}, TYPE  {5} ) ;; CREATE TEMPORARY TABLE RES_TMP ( ENTRYID {6}, VALUE {7}({4}) {3} ) ;; CREATE INDEX RES_TMP1 ON RES_TMP(VALUE,ENTRYID);; CREATE TEMPORARY TABLE LIT_TMP ( ENTRYID {6}, VALUE  {7}({4}) {3}, MODIFIER_ID {6} ) ;; CREATE INDEX LIT_TMP1 ON LIT_TMP(VALUE,MODIFIER_ID,ENTRYID);; CREATE TEMPORARY TABLE STMTS_TMP ( OPERATION   {5} NOT NULL, ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL, METADATA   {5} NOT NULL, UUID    {6} NOT NULL, NAMEDGRAPHID  {6} NOT NULL, SUBJECT         {6} NOT NULL, PREDICATE        {6} NOT NULL, OBJECT          {6} NOT NULL, REVISION   {6} , COMMITTED   {6} NOT NULL ) ;; CREATE INDEX STMTS1 ON STMTS_TMP(OPERATION,ID);; CREATE INDEX STMTS2 ON STMTS_TMP(ID,OPERATION);; CREATE INDEX STMTS3 ON STMTS_TMP(STMTID,ID);; CREATE TEMPORARY TABLE STMT_ID_TMP ( ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL ) ;; CREATE INDEX STMT_ID_1 ON STMT_ID_TMP(STMTID,ID);; CREATE LOCAL TEMPORARY TABLE REMOVE_GRAPHS_TMP ( ID     {6} NOT NULL, TYPE    {5} NOT NULL, REND    {6} NOT NULL ) ;; CREATE INDEX RG_ID ON REMOVE_GRAPHS_TMP(TYPE,ID,REND);; 
