...
initIndexes=CREATE INDEX SRUUID ON STATEMENTS (UUID,COMMITTED,REND,RSTART);; CREATE INDEX SRHS ON STATEMENTS (SUBJECT,PREDICATE);; CREATE INDEX SRHPO ON STATEMENTS (PREDICATE,OBJECT);; CREATE INDEX SRHO ON STATEMENTS (OBJECT);; CREATE INDEX SRHC ON STATEMENTS (COMMITTED,ID,REND);; CREATE INDEX SRIE ON STATEMENTS (ID,REND);; CREATE INDEX SNG ON STATEMENTS (COMMITTED,NAMEDGRAPHID,REND) ;; CREATE INDEX SRHSSPOIO ON STATEMENTS (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT,METADATA,REND,RSTART,ID);; CREATE INDEX SRHSSPOIO_NR ON STATEMENTS_NR (NAMEDGRAPHID,SUBJECT,PREDICATE,OBJECT,METADATA,ID);; CREATE INDEX SRHS_NR ON STATEMENTS_NR (SUBJECT);; CREATE INDEX SRHP_NR ON STATEMENTS_NR (PREDICATE);; CREATE INDEX SRHO_NR ON STATEMENTS_NR (OBJECT);; CREATE INDEX SRHC_NR ON STATEMENTS_NR (COMMITTED);; CREATE INDEX NG_ITRM ON NAMEDGRAPHS (ID,COMMITTED,HEND,HSTART,REVISION,METAID);; CREATE INDEX NG_UUDI ON NAMEDGRAPHS (COMMITTED,ID,HEND,UUID);; CREATE INDEX NG_MH ON NAMEDGRAPHS (METAID,HEND,HSTART, ID);; CREATE INDEX NG_RHI ON NAMEDGRAPHS(REVISION,HSTART,ID);; CREATE INDEX NG_UUID ON NAMEDGRAPHS(UUID,HEND,ID);; CREATE INDEX NG_LM ON NAMEDGRAPHS(LASTMODIFIEDBY);; CREATE INDEX NG_IHR_NR ON NAMEDGRAPHS_NR (ID,COMMITTED,HSTART,REVISION);; CREATE INDEX NG_C_NR ON NAMEDGRAPHS (COMMITTED,ID);; CREATE INDEX NG_M_NR ON NAMEDGRAPHS_NR (METAID);; CREATE INDEX NG_RH_NR ON NAMEDGRAPHS_NR(REVISION,HSTART);; CREATE INDEX NG_UUIDR ON NAMEDGRAPHS_NR(UUID,ID);; CREATE INDEX NG_LMR ON NAMEDGRAPHS_NR(LASTMODIFIEDBY);; CREATE INDEX LTT ON LASTTRANSACTIONTIME (ID,COMMITED);; CREATE INDEX LTT4 ON LASTTRANSACTIONTIME (COMMITED,ID);; CREATE INDEX TID ON TRANSACTIONTIME (ID,COMMITED);; CREATE INDEX TCIU ON TRANSACTIONTIME (COMMITED,ID,URI);; CREATE INDEX TCSID ON TRANSACTIONTIME(SERVERID,COMMITED);; CREATE UNIQUE INDEX {0}_U_V ON {0}_U(VALUE);; CREATE INDEX {0}_U_RI ON {0}_U(REF DESC,ID);; CREATE INDEX {0}_U_RV ON {0}_U(REF,VALUE,ID);; CREATE INDEX {0}_LU_V ON {0}_LU(HASH,ID);; CREATE INDEX {0}_LU_RI ON {0}_LU(REF DESC,ID);; CREATE UNIQUE INDEX {0}_L_V ON {0}_L(VALUE,MODIFIER_ID);; CREATE INDEX {0}_L_RI ON {0}_L(REF DESC,ID);; CREATE INDEX {0}_L_RV ON {0}_L(REF,MODIFIER_ID,VALUE,ID);; CREATE INDEX {0}_LL_V ON {0}_LL(HASH,MODIFIER_ID);; CREATE INDEX {0}_LL_RI ON {0}_LL(REF DESC,ID);; CREATE UNIQUE INDEX {0}_TL_V ON {0}_TL(VALUE,MODIFIER_ID);; CREATE INDEX {0}_TL_RI ON {0}_TL(REF DESC,ID);; CREATE INDEX {0}_TL_RV ON {0}_TL(REF,MODIFIER_ID,VALUE,ID);; CREATE INDEX {0}_LTL_V ON {0}_LTL(HASH,MODIFIER_ID);; CREATE INDEX {0}_LTL_RI ON {0}_LTL(REF DESC,ID);; CREATE UNIQUE INDEX {0}_B_V ON {0}_B(VALUE);; CREATE INDEX {0}_B_RI ON {0}_B(REF DESC,ID);; CREATE INDEX {0}_B_RV ON {0}_B(REF,VALUE,ID);; CREATE INDEX {0}_UIDS_TI ON  {0}_USED_IDS(TRANSACTIONID,ID)   {1};; CREATE INDEX {0}_UIDS_IT ON  {0}_USED_IDS(ID,TRANSACTIONID)   {1};; CREATE UNIQUE INDEX LOCKEDS ON LOCKED_GRAPHS(TRANSACTIONID,ID);; CREATE UNIQUE INDEX QG_DSID_ID ON QUERY_GRAPHS(DSID,ID);; 
...
initTablespace=CREATE TEMPORARY TABLESPACE usr_tbsp TEMPFILE ''crt_usr_tbsp.dbf'' SIZE 50M AUTOEXTEND ON;; 
createTemporaryTables=CREATE GLOBAL TEMPORARY TABLE DEFAULTGRAPHS_TMP ( ID {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE NAMEDGRAPHS_TMP ( ID {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMPGRAPHS ( ID {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMP_COLUMNS( C0    {6}, C1    {6}, C2    {6}, C3    {6}, C4    {6}, C5    {6}, C6    {6}, C7    {6}, C8    {6}, C9    {6}, C10    {6}, C11    {6}, C12    {6}, C13    {6}, C14    {6}, C15    {6} )ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMP_CONSTRAINT0( ID    {6} )ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMP_CONSTRAINT1( ID    {6} )ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMP_CONSTRAINT2( ID    {6} )ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE TEMP_CONSTRAINT3( ID    {6} )ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE NGR_TMP( ID     {6}, METAID   {6}, REVISION   {6}, NEW_REVISION {6}, UUID   {6} ) ON COMMIT PRESERVE ROWS ;; CREATE GLOBAL TEMPORARY TABLE STMTS_REP_TMP ( ID     {7}({4}) {3} NOT NULL, NAMEDGRAPHID  {6}, SUBJECT       {6}, PREDICATE     {6}, OBJECT        {6}, RSTART    {6}, REND    {6} ) ON COMMIT PRESERVE ROWS ;; CREATE GLOBAL TEMPORARY TABLE SUBJECT_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE GLOBAL TEMPORARY TABLE PREDICATE_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE GLOBAL TEMPORARY TABLE OBJECT_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE GLOBAL TEMPORARY TABLE NAMEDGRAPH_IDS_TEMP ( ID  {6} )ON COMMIT DELETE ROWS;; CREATE GLOBAL TEMPORARY TABLE ID_TMP ( ID  {6}, ENTRYID {6}, TYPE  {5} ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE RES_TMP ( ENTRYID {6}, VALUE {7}({4}) {3} ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE LIT_TMP ( ENTRYID {6}, VALUE  {7}({4}) {3}, MODIFIER_ID {6} ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE STMTS_TMP ( OPERATION   {5} NOT NULL, ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL, METADATA   {5} NOT NULL, UUID    {6} NOT NULL, NAMEDGRAPHID  {6} NOT NULL, SUBJECT       {6} NOT NULL, PREDICATE     {6} NOT NULL, OBJECT           {6} NOT NULL, REVISION   {6} NOT NULL, COMMITTED   {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE STMT_ID_TMP ( ID     {7}({4}) {3} NOT NULL, STMTID    {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; CREATE GLOBAL TEMPORARY TABLE REMOVE_GRAPHS_TMP ( ID     {6} NOT NULL, TYPE    {5} NOT NULL, REND    {6} NOT NULL ) ON COMMIT PRESERVE ROWS;; 
...
5To6=ALTER TABLE STATEMENTS ADD UUID {6};; CREATE INDEX SRUUID ON STATEMENTS (UUID);; UPDATE STATEMENTS SET UUID=(SELECT DISTINCT UUID FROM NAMEDGRAPHS WHERE NAMEDGRAPHS.HEND IS NULL AND (NAMEDGRAPHS.ID=STATEMENTS.NAMEDGRAPHID OR NAMEDGRAPHS.METAID=STATEMENTS.NAMEDGRAPHID));; ALTER TABLE STATEMENTS MODIFY  UUID NUMBER NOT NULL;; 
5To6more=
6To7=ALTER TABLE SERVER DROP COLUMN INITIALIZED;; ALTER TABLE SERVER ADD INITIALIZED {6};; UPDATE SERVER SET INITIALIZED=1;; 
6To7more=
7To8=ALTER TABLE {0}_DATATYPE  MODIFY ID {6};; ALTER TABLE {0}_LANGUAGE  MODIFY ID {6};; CREATE SEQUENCE LANG_SEQ    START WITH 1 INCREMENT BY 1 CACHE 20000;; CREATE SEQUENCE DATATYPE_SEQ  START WITH 1 INCREMENT BY 1 CACHE 20000;; 
7To8more=
8To9=ALTER TABLE QUERY_GRAPHS DROP COLUMN SERVERID;; 
8To9more=
10To11=
10To11more=
...
InsertStatements.commitTransactionAddStatementsNR=UPDATE /*+ index(STATEMENTS_NR,SRHC_NR) */ STATEMENTS_NR SET COMMITTED=0 WHERE COMMITTED=?
...
InsertStatements.abortTransactionAddStatementsNRRange=DELETE /*+ index(STATEMENTS_NR,SRHC_NR) */ FROM STATEMENTS_NR WHERE COMMITTED=? AND STATEMENTS.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ? )
...
InsertStatements.abortTransactionRemoveStatements=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS SET REND=CAST(NULL AS {0}),COMMITTED=0 WHERE COMMITTED=?
...
InsertStatements.purgeInsertStatements=DELETE FROM {0}STMTS_TMP WHERE {0}STMTS_TMP.OPERATION=1 AND EXISTS (SELECT /*+ index(S,SRHC) */ NULL FROM STATEMENTS S WHERE S.ID={0}STMTS_TMP.ID AND S.COMMITTED=0 AND S.REND IS NULL)
InsertStatements.abortTransactionAddNamedGraphs=DELETE FROM NAMEDGRAPHS WHERE COMMITTED=?
InsertStatements.commitTransactionAddNamedGraphsNR=UPDATE /*+ index(NAMEDGRAPHS_NR,NG_C_NR) */ NAMEDGRAPHS_NR SET COMMITTED=0 WHERE COMMITTED=?
InsertStatements.abortTransactionRemoveStatementsNRRange=UPDATE /*+ index(STATEMENTS_NR,SRHC_NR) */ STATEMENTS_NR SET COMMITTED=0 WHERE COMMITTED=? AND STATEMENTS.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ?)
InsertStatements.commitTransactionRemoveStatementsNRRange=DELETE /*+ index(STATEMENTS_NR,SRHC_NR) */ FROM STATEMENTS_NR WHERE COMMITTED=? AND STATEMENTS_NR.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ?)
InsertStatements.abortTransactionAddNamedGraphsNR=DELETE FROM NAMEDGRAPHS_NR WHERE COMMITTED=?
InsertStatements.commitRemoveStatements=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS SET COMMITTED=?, REND=(SELECT ST.REVISION FROM {0}STMTS_TMP ST WHERE ST.OPERATION=0 AND ST.ID=STATEMENTS.ID) WHERE COMMITTED=0 AND REND IS NULL AND ID IN (SELECT ST.ID FROM {0}STMTS_TMP ST WHERE ST.OPERATION=0)
...
InsertStatements.commitTransactionStatements=UPDATE /*+ index(STATEMENTS,SNG) */ STATEMENTS SET COMMITTED=0 WHERE COMMITTED=?
InsertStatements.commitRemoveStatementsRange=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS SET COMMITTED=?, REND=(SELECT ST.REVISION FROM {0}STMTS_TMP ST WHERE ST.OPERATION=0 AND ST.ID=STATEMENTS.ID) WHERE COMMITTED=0 AND REND IS NULL AND ID IN (SELECT ST.ID FROM {0}STMTS_TMP ST WHERE ST.STMTID >= ? AND ST.STMTID < ? AND ST.OPERATION=0)
...
InsertStatements.abortTransactionAddStatements=DELETE /*+ index(STATEMENTS,SRHC) */ FROM STATEMENTS WHERE COMMITTED=?
InsertStatements.countInsertStatements=SELECT COUNT(1) FROM {0}STMTS_TMP WHERE OPERATION=1
InsertStatements.commitTransactionStatementsRange=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS SET COMMITTED=0 WHERE COMMITTED=? AND STATEMENTS.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ? )
InsertStatements.abortTransactionAddStatementsNR=DELETE /*+ index(STATEMENTS_NR,SRHC_NR) */ FROM STATEMENTS_NR WHERE COMMITTED=?
...
InsertStatements.abortTransactionAddStatementsRange=DELETE /*+ index(STATEMENTS,SRHC) */ FROM STATEMENTS WHERE COMMITTED=? AND STATEMENTS.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ?)
...
InsertStatements.commitRemoveStatementsNR=UPDATE /*+ index(STATEMENTS_NR,SRHC_NR) */ STATEMENTS_NR SET COMMITTED=? WHERE COMMITTED=0 AND ID IN( SELECT ST.ID FROM {0}STMTS_TMP ST WHERE ST.OPERATION=0 )
InsertStatements.purgeRemoveStatementsNR=DELETE FROM {0}STMTS_TMP WHERE {0}STMTS_TMP.OPERATION=0 AND NOT EXISTS (SELECT /*+ index(STATEMENTS_NR,SRHC_NR) */ NULL FROM STATEMENTS_NR S WHERE S.ID={0}STMTS_TMP.ID AND S.COMMITTED=0)
InsertStatements.commitTransactionNamedGraphs=UPDATE /*+ index(NAMEDGRAPHS,NG_UUDI) */ NAMEDGRAPHS SET COMMITTED=0 WHERE COMMITTED=?
InsertStatements.purgeInsertStatementsNR=DELETE FROM {0}STMTS_TMP WHERE {0}STMTS_TMP.OPERATION=1 AND EXISTS (SELECT /*+ index(STATEMENTS_NR,SRHC_NR) */ NULL FROM STATEMENTS_NR S WHERE S.ID={0}STMTS_TMP.ID AND S.COMMITTED=0)
InsertStatements.commitRemoveStatementsNRRange=UPDATE /*+ index(STATEMENTS_NR,SRHC_NR) */ STATEMENTS_NR SET COMMITTED=? WHERE COMMITTED=0 AND ID IN( SELECT ST.ID FROM {0}STMTS_TMP ST WHERE ST.STMTID >= ? AND ST.STMTID < ? AND ST.OPERATION=0)
...
InsertStatements.commitTransactionRemoveNamedGraphsNR=DELETE /*+ index(NAMEDGRAPHS_NR,NG_C_NR) */ FROM NAMEDGRAPHS_NR WHERE COMMITTED=?
InsertStatements.commitTransactionAddStatementsNRRange=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS_NR SET COMMITTED=0 WHERE COMMITTED=? AND STATEMENTS_NR.ID IN (SELECT ST.ID FROM {0}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ?)
InsertStatements.purgeRemoveStatements=DELETE FROM {0}STMTS_TMP WHERE {0}STMTS_TMP.OPERATION=0 AND NOT EXISTS(SELECT /*+ index(S,SRHC) */ NULL FROM STATEMENTS S WHERE S.ID={0}STMTS_TMP.ID AND S.COMMITTED=0 AND S.REND IS NULL)
InsertStatements.abortTransactionRemoveStatementsNR=UPDATE /*+ index(STATEMENTS_NR,SRHC_NR) */ STATEMENTS_NR SET COMMITTED=0 WHERE COMMITTED=?
InsertStatements.abortTransactionRemoveStatementsRange=UPDATE /*+ index(STATEMENTS,SRHC) */ STATEMENTS SET REND=CAST(NULL AS {0}),COMMITTED=0 WHERE COMMITTED=? AND ST.ID=STATEMENTS.ID IN (SELECT ST.ID FROM {1}STMT_ID_TMP ST WHERE ST.STMTID>=? AND ST.STMTID < ?)
InsertStatements.commitTransactionRemoveStatementsNR=DELETE /*+ index(STATEMENTS_NR,SRHC_NR) */ FROM STATEMENTS_NR WHERE COMMITTED=?
...
NamedGraph.getAllRevisionedNamedGraphs=SELECT ID FROM NAMEDGRAPHS WHERE HEND IS NULL AND COMMITTED=0 UNION SELECT METAID FROM NAMEDGRAPHS WHERE HEND IS NULL AND COMMITTED=0
...
NamedGraph.getBatchUUIDForNamedGraph=SELECT UUID,ID FROM NAMEDGRAPHS WHERE ID IN (SELECT ID FROM {0}{1}) AND HEND IS NULL AND COMMITTED=0)
NamedGraph.getUUIDForNamedGraph=SELECT UUID FROM NAMEDGRAPHS WHERE ID=? AND HEND IS NULL AND COMMITTED=0
NamedGraph.containsMetadataGraphNRAtRevision=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS_NR WHERE METAID=? AND REVISION=?
...
NamedGraph.countAllRevisionedNamedGraphs=SELECT COUNT(1) FROM NAMEDGRAPHS WHERE HEND IS NULL AND COMMITTED=0
...
NamedGraph.containsNamedGraphNonRevisioned=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS_NR WHERE ID = ? 
...
NamedGraph.containsNamedGraphNRAtRevision=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS_NR WHERE ID = ? AND REVISION=?
NamedGraph.containsNamedGraphAtRevision=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS WHERE UUID=? AND REVISION=? AND COMMITTED<=0
...
NamedGraph.selectNamedGraphRevisioned=SELECT ID,METAID,UUID,REVISION,HSTART,LASTMODIFIEDBY FROM NAMEDGRAPHS WHERE ID = ? AND HEND IS NULL AND COMMITTED=0
NamedGraph.deleteNamedGraph=UPDATE NAMEDGRAPHS SET HEND=?,COMMITTED=? WHERE ID = ? AND HEND IS NULL
NamedGraph.getBatchNamedGraphForUUID=SELECT UUID,ID FROM NAMEDGRAPHS WHERE UUID IN (SELECT ID FROM {0}{1}) AND HEND IS NULL AND COMMITTED=0)
...
NamedGraph.containsMetadataGraphRevisioned=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS WHERE METAID=? AND HEND IS NULL AND COMMITTED=0
NamedGraph.selectNamedGraphRevisionedBatch=SELECT ID,METAID,UUID,REVISION,HSTART,LASTMODIFIEDBY FROM NAMEDGRAPHS WHERE ID IN (SELECT ID FROM {0}{1}) AND HEND IS NULL AND COMMITTED=0)
...
NamedGraph.containsNamedGraphRevisioned=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS WHERE ID = ? AND HEND IS NULL AND COMMITTED=0
...
NamedGraph.containsMetadataGraphNonRevisioned=SELECT /*+ opt_param('optimizer_mode','first_rows_1') */ ID FROM NAMEDGRAPHS_NR WHERE METAID=?
NamedGraph.deleteStatementsForNamedGraphBatch=UPDATE STATEMENTS SET REND=(SELECT {0}{1}.REND FROM {0}{1} WHERE {0}{1}.TYPE=0 AND {0}{1}.ID=STATEMENTS.NAMEDGRAPHID) WHERE REND IS NULL AND NAMEDGRAPHID IN (SELECT {0}{1}.ID FROM {0}{1} WHERE {0}{1}.TYPE=0)
...
Statement.findLiteralStatementsLimited=SELECT NAMEDGRAPHID, SUBJECT, PREDICATE, OBJECT FROM( SELECT ROW_NUMBER() OVER (ORDER BY NAMEDGRAPHID) AS ROWNUMBER,NAMEDGRAPHID, SUBJECT, PREDICATE, OBJECT FROM STATEMENTS WHERE COMMITTED=0 AND REND IS NULL AND OBJECT > 2305843009213693953 and OBJECT < 6917529027641081855) WHERE ROWNUMBER > ? AND ROWNUMBER<= ?
Statement.findLiteralStatementsNRLimited=SELECT NAMEDGRAPHID, SUBJECT, PREDICATE, OBJECT FROM( SELECT ROW_NUMBER() OVER (ORDER BY NAMEDGRAPHID) AS ROWNUMBER,NAMEDGRAPHID, SUBJECT, PREDICATE, OBJECT FROM STATEMENTS_NR WHERE COMMITTED=0 AND OBJECT > 2305843009213693953 and OBJECT < 6917529027641081855) WHERE ROWNUMBER > ? AND ROWNUMBER<= ?
...
Server.stats=ANALYZE TABLE {0} COMPUTE STATISTICS
