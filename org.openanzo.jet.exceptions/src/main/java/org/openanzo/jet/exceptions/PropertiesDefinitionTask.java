...
import org.openanzo.jet.exceptions.templates.PropertiesDefinitionsTemplate;
import org.openanzo.jet.exceptions.templates.PropertiesDefinitionsWikiTemplate;
 ...
 * 
 */
public class PropertiesDefinitionTask extends MatchingTask {
    ...
    public void parseFile(String fileName) {
        try {
            ...
            PropertiesDefinitionsTemplate template = new PropertiesDefinitionsTemplate();
            PropertiesDefinitionsWikiTemplate wikiTemplate = new PropertiesDefinitionsWikiTemplate();
            for (PropertyGroup group : parser.getGroups()) {
                String output = template.generate(group);
                String wikiOutput = wikiTemplate.generate(group);
                File destFile = new File(this.destinationFolder, group.getPackageName().replace('.', File.separatorChar) + File.separatorChar + group.getClassName() + "Properties.java");
                if (!destFile.exists()) {
